...
def add_tp():
    ...
    if request.method == 'POST':
        ...
        if form.get('settings') != "":
            new_tp = Tp(session['id'], form.get('name'), form.get('options'), str(form.get('priority')), str(form.get('settings').split(',')))
        else:
            new_tp = Tp(session['id'], form.get('name'), form.get('options'), str(form.get('priority')))
...
def view_tp(tp_id):
    ...
    eve = Dat.query.filter_by(tp_id = str(tp_id),details='evening').all()
